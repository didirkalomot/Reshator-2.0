...
class Div(Infix, Operator):
    ...
    # ИЗМЕНЕНИЕ: Добавлен метод __iter__ для поддержки вертикального отображения дроби
    def __iter__(self):
...
class Pow(Infix, Operator):
    ...
    # ИЗМЕНЕНИЕ: Добавлен метод __iter__ для поддержки отображения степени (поднятие показателя)
    def __iter__(self):
...
class Log(Prefix, Operator):
    ...
    # ИЗМЕНЕНИЕ: Переопределен метод __iter__ для поддержки отображения логарифма
    def __iter__(self):
        arg_begin, arg_end = tokens.layout_bounds.create_pair()
        yield self
        yield arg_begin
        yield from self.two
        yield arg_end
